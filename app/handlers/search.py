...
async def search(call: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    """Обработчик перехода к поиску."""
    user = await user_service.get_user_with_auto(session, tg_id=call.from_user.id)
    if user:
        if user.autos:
...
async def enter_search_number(message: Message, session: AsyncSession, state: FSMContext) -> None:
